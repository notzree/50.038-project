...
def compute_human_features(row: Mapping[str, object]) -> dict[str, float | str]:
    """Compute Spotify-like human-readable proxy features from extracted audio stats.

    These are heuristic proxies for presentation and downstream modeling. They are
    intentionally bounded to [0, 1] and should be treated as approximate signals.
    """
    ...
    # Optional richer fields from full feature set
    ...
    # Reuse extracted engineered features when available; fallback to heuristics.
    ...
    # Very rough vocal/speech proxy from zero-crossing + onset behavior.
    ...
    # Approximate valence from timbral profile; mainly for interpretability.
    valence_proxy = _norm(
        0.30 * tempo_n
